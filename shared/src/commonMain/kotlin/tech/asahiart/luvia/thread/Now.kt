...
public data class DoneLedger(
    ...
    public fun observe(threads: List<Thread>, nowMs: Long): DoneLedger {
    ...
    public fun markViewed(thread: Thread): DoneLedger = copy(viewed = viewed + key(thread))

    public fun isRecent(thread: Thread, nowMs: Long): Boolean {
        val k = key(thread)
        val since = firstSeenMs[k] ?: return false
        return k !in viewed && nowMs - since < ttlMs
    }

    private fun key(thread: Thread) = "${thread.hostId}/${thread.id}"
...
public fun buildNowState(hosts: List<NowHost>, ledger: DoneLedger, nowMs: Long): NowState {
    fun group(host: NowHost, pick: (Thread) -> Boolean): NowGroup? {
