...
pub fn inject_auth(payload: &[u8], token: &str) -> Result<Vec<u8>> {
    if token.is_empty() || token.len() > 256 || !token.bytes().all(|byte| byte.is_ascii_graphic()) {
        return Err(Error::new("auth_failed", "minted token is not a valid UHP secret"));
    }
    reject_duplicate_keys(payload)?;
    let mut value: Value = serde_json::from_slice(payload)?;
    let object = value.as_object_mut().ok_or_else(|| {
        Error::new("invalid_request", "UHP request must be a JSON object")
    })?;
...
pub fn mint_token(path: &Path, evidence: Evidence, scopes: &[&str]) -> Result<MintedToken> {
    if scopes.is_empty() {
        return Err(Error::new("auth_failed", "delegated token scopes are empty"));
    }
    ...
    let result = value.get("result").ok_or_else(|| {
        Error::new("auth_failed", "token create returned no result")
    })?;
    let secret = result
        .get("token")
        .and_then(Value::as_str)
    ...
    if secret.is_empty() || id.is_empty() {
        return Err(Error::new("auth_failed", "token create returned an empty token"));
    }
    Ok(MintedToken {
        id: id.to_string(),
